...
export default async function TradingPlanPage() {
  ...
  if (!rules) {
    return (
      <div>
        <PageHeader eyebrow="Trading Plan" title="Trading plan status" />
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Maximum trades per day</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Maximum open positions</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          {status.sessions.filter((s) => s.enabled).length === 0 ? (
            ...
            <ul className="space-y-3">
              {status.sessions
                .filter((s) => s.enabled)
                .map((session) => (
                  <li key={session.key} className="flex items-center justify-between">
